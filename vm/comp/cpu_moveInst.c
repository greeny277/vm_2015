case 0x88:
	...
	if(!cpu_decode_RM(cpu_state, &s_op, EIGHT_BIT)){
		...
		if(s_op.regmem_type == MEMORY){
			cpu_write_byte_in_ram(cpu_state, src, s_op.regmem_mem);
		} else {
...
case 0x89:
	...
	if(!cpu_decode_RM(cpu_state, &s_op, !EIGHT_BIT)){
		...
		if(s_op.regmem_type == MEMORY){
			cpu_write_word_in_ram(cpu_state, src, s_op.regmem_mem);
		} else {
...
case 0x8A:
	...
	if(!cpu_decode_RM(cpu_state, &s_op, EIGHT_BIT)){
		uint8_t src;
		if(s_op.regmem_type == MEMORY){
			src = cpu_peek_byte_from_ram(cpu_state, s_op.regmem_mem);
		} else {
...
case 0x8B:
	...
	if(!cpu_decode_RM(cpu_state, &s_op, !EIGHT_BIT)){
		uint32_t src;
		if(s_op.regmem_type == MEMORY){
			src = cpu_peek_word_from_ram(cpu_state, s_op.regmem_mem);
		} else {
...
case 0xC6:
	/* Copy imm8 to r/m8. */
	if(!cpu_decode_RM(cpu_state, &s_op, EIGHT_BIT)){
		uint8_t src = cpu_read_byte_from_ram(cpu_state);
		if(s_op.regmem_type == MEMORY){
			cpu_write_byte_in_ram(cpu_state, src, s_op.regmem_mem);
		} else {
			cpu_write_byte_in_reg(src, s_op.regmem_reg, s_op.regmem_type == REGISTER_HIGH);
...
case 0xC7:
	/* Copy imm32 to r/m32. */
	if(!cpu_decode_RM(cpu_state, &s_op, !EIGHT_BIT)){
		uint8_t src = cpu_read_word_from_ram(cpu_state);
		if(s_op.regmem_type == MEMORY){
			cpu_write_word_in_ram(cpu_state, src, s_op.regmem_mem);
		} else {
			cpu_write_word_in_reg(src, s_op.regmem_reg);

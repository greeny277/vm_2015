...
case 4: {
	#ifdef DEBUG_PRINT_INST
	cpu_print_inst("AND r/m32, imm8\n");
	#endif
	
	int32_t imm = (int32_t) ((int8_t) cpu_consume_byte_from_mem(cpu_state));
	
	int32_t op1;
	if(s_op.regmem_type == MEMORY)
		op1 = (int32_t) cpu_read_doubleword_from_mem(cpu_state, s_op.regmem_mem, DATA);
	else
		op1 = (int32_t) cpu_read_doubleword_from_reg(s_op.regmem_reg);

	int32_t result = imm & op1;
